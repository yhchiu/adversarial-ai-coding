...
| `IMPL_ARGS` | 空 | 實作 slot 的額外 CLI 參數。內建 agent 附加在該指令的 `CLAUDE_ARGS` / `CODEX_ARGS` / `AGY_ARGS` 之後；custom agent 則取代 slot args。內建 agent **不得**在此帶模型旗標（保留字，見下）—— 模型一律走 `IMPL_MODEL`；custom agent 反之，模型只能靠這裡帶。 |
...
| `CODEX_ARGS` | codex 規則（現有 + 新的 model 規則） |
| `AGY_ARGS` | agy 規則（現有 + 新的 model 規則） |
| `AGENT_A_ARGS` / `AGENT_B_ARGS` | 只檢查引號（custom 契約是自由參數，現有行為 —— custom agent 本來就只能靠 args 帶模型） |
...
各 adapter 的保留旗標（claude 與 codex 的清單都用 `--help` 實測過）：

- **claude**：session —— `-c` / `--continue`、`-r` / `--resume`、`--session-id`、`--fork-session`、
  `--no-session-persistence`、`--from-pr`；輸出契約 —— `--output-format`、`--json-schema`。
- **codex**：`--json`、`resume`、`--sandbox` / `-s`、`-c sandbox_mode=`。
- **agy**：`--log-file`、`--continue`、`--conversation`。
- **model（三個內建 agent 共通，新增）**：`--model`、`-m`（codex 有 `-m/--model` 兩種寫法）、
  以及 codex 的 `-c model=` / `--config model=`。錯誤訊息直接指路：
  `CLAUDE_ARGS cannot set the model; use MODEL_A / MODEL_B / IMPL_MODEL instead`。

  理由：adapter 把生成的 model 旗標放在額外 args **前面**，所以 args 裡的 model 旗標會靜默贏過
  `MODEL_A` / `IMPL_MODEL`，而 metrics.csv 的 `model` 欄仍記著被蓋掉的那一個。對一個以成本歸因為
  核心的功能，metadata 說謊是致命的。保留這些旗標 = 模型只剩一條設定路徑，metadata 永遠不會說謊。
  custom agent 不受影響（它們沒有 `MODEL_*` 可用，模型本來就只能放 args）。

所有規則都必須同時處理 `--flag value` 與 `--flag=value` 兩種形式（現有的 codex `--sandbox=`
檢查已是此模式）。
...
- **`workflow.py`**：`write-code` stage(:691-727) 進迴圈前建一次
  ...
  的警告也走 `ctx.log()`。
- **`archive.py`**：
  - `write_meta` 的 payload 與 `log_section` 的 banner 加 `agent_slot`；`METRICS_HEADER` 尾端
    追加 `agent_slot` 欄（**追加在最後**，才不會動到 `metrics_summary` 的既有欄位索引）。
    這讓「實作 slot 真的有跑」可以被稽核，即使它與 owner 同指令同模型。
  - `write_run_metadata`（:214）與 `write_log_metadata`（:236）加上 **requested**（原始未解析）的
    `impl_agent` / `impl_model` / `impl_args`。這條不能省：`establish_run_archive`（:362-368）在
    resume 時因原目錄已存在，會開一個**新的** `<run_id>-2` 目錄；若 `write-code` 早已完成，
    workflow 那行 resolved 設定 log 在新 archive 裡根本不會印，新 archive 就會完全沒有實作 slot
    的痕跡。requested config 走 run metadata（啟動時就寫），resolved ref 走 workflow log
    （dual-spec 選完 owner 之後才印）—— 兩者刻意分開，不互相冒充。
...
- **文件**：`README.md` / `README.zh-TW.md` 加設定表格三列、"Strong model plans, cheap model
  implements" 用法段落、stage 5 說明、custom 指令必須不同名的限制、保留旗標政策（session／輸出／
  **模型**：內建 agent 的模型只能走 `MODEL_A` / `MODEL_B` / `IMPL_MODEL`）、「換手即丟棄」的
  session 語意、resume 可覆寫不可清空與清空的操作方式。
  `docs/python-port-parity.md` 記兩行：實作 slot 是 Python port 新增、bash 沒有的概念；
  內建 agent 的 args 不再接受模型旗標（bash 沒有這道驗證）。

## Commits

每個 commit 先跑自己的 targeted tests，再跑完整 suite，綠了才 commit。

1. **`fix(agents): reserve workflow-owned flags in built-in agent args`**
   把 `_validate_reserved_args` 改成依 adapter 套規則：補上 claude 的 session／輸出保留旗標，
   並對三個內建 agent 一律保留 model 旗標。這修掉兩個既有漏洞 —— `CLAUDE_ARGS='--continue'`
   今天就能架空剛落地的 exact-ID resume；`CODEX_ARGS='-m gpt-5'` 今天就能讓 metrics.csv 的
   `model` 欄說謊。獨立 commit，本身就有價值，也是 `IMPL_ARGS` 依 adapter 驗證的前提。
...
6. **`feat(workflow): route plan tasks through the implementation slot`**
   ...
   `agent_slot` 進 meta / log banner / metrics、requested `impl_*` 進 run metadata。
   targeted：`test_stageflow.py`、`test_archive_io.py`、
...
- **`test_agents.py`**
  ...
  - 保留參數：`CLAUDE_ARGS='--continue'` / `--output-format text` /
    `--no-session-persistence` → raise；三個內建 agent 的 `--model` / `-m` / `-c model=` → raise
    且錯誤訊息指向 `MODEL_A` / `MODEL_B` / `IMPL_MODEL`；`CODEX_ARGS='-c model_reasoning_effort=low'`
    → 放行；`AGENT_A_ARGS='--model pro'`（custom）→ 放行；`IMPL_ARGS` 依 `IMPL_AGENT` 指向的
    adapter 判定（custom agent 用到碰巧同名的 `resume` / `--json` **不得**被誤擋）。
    每條規則都要測 `--flag value` 與 `--flag=value` 兩種形式。
...
- **offline fake built-in（新）**：在暫存目錄放一個名為 `codex` 的假執行檔並塞進 PATH
  ...
  （custom agent 依設計忽略 `IMPL_MODEL`，也不走內建的 argv builder）。
  **換模型的 resume 案例也放這裡**，因為只有這裡的 shim 觀察得到 model 旗標。
- **`test_resume_integration.py`**：
  ...
  - mid-queue resume：第一個 task 後中止，resume 時把 `IMPL_AGENT` 換成**第二個 wrapper**
    （`fake-impl2`），斷言只有剩下的 task 由它實作、已完成的 task 不重跑。
    **不能用「換 `IMPL_MODEL`」當斷言** —— custom fake agent 依設計忽略模型，那樣什麼也證明不了。
