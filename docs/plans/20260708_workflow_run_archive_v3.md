...
- **mkdir 必須同時建立 live 工作目錄與 archive 目錄(見 review #8-1)**:`setup_workspace` 後用 `mkdir -p "$WF" "$WF_RUN/logs" "$SPEC_DIR"`。不能只建 `$WF_RUN/logs`,因為 `RUNS_DIR` 可能指到 `.workflow` 之外,但 live 檔(`$WF/review.md`、`$WF/verdict.json`、`$WF/.gitignore`、`ENGINE_OUT`)仍需要 `$WF` 存在。
...
- **metrics row 必須做 CSV escaping(見 review #8-3)**:`model_args` 可能包含空白、逗號、雙引號或 shell 片段,不可用裸 `echo "$a,$b,..."`。新增 `csv_row`/`write_csv_row` helper,用 `jq -r @csv` 或等價安全轉義產生每列;測試需覆蓋 `CODEX_ARGS='-c model="x,y" --flag "quoted"'` 這類含逗號與引號的輸入。
- **log 檔本身也要有 metadata(見 review #8-2)**:建立 `$LOG.meta.json`(即 `$WF_RUN/logs/001-run.log.meta.json`),欄位至少包含 `generated_at`、`generator_role:"workflow"`、`engine_a`/`model_a`/`args_a`、`engine_b`/`model_b`/`args_b`、`run_id`、`log_path`。`NNN-run-metadata.json` 也要列出 log 路徑,但不能只依賴 run metadata,避免單獨搬動 log 檔時 attribution 遺失。
...
- **每個 retry attempt 都各自落檔(見 review #5/#8-4)**:`engine_call` 每次呼叫引擎後,立即把當次 `ENGINE_OUT` 快照成 `NNN-<role>-attempt-<n>-rc<rc>.raw`,不讓限額/失敗 attempt 的原始輸出被下一次覆寫;最終成功輸出另存 `.raw`。`w_claude`/`r_claude` 成功時 `$out` 也寫入 `ENGINE_OUT` 以供擷取。
  - **明確改 `engine_call` 介面,不要靠隱含全域猜 role/stage**:`engine_call <role> <engine> <artifact_slug> <engine_fn> <prompt>`。`role` 用於 metadata 與檔名,`engine` 用於解析 model/model_args,`artifact_slug` 由 caller 傳入(例如 `worker-<stage>-r<round>` / `reviewer-<stage>-r<round>`),`engine_fn` 才是實際 `w_*`/`r_*` 函式。這樣 `engine_call` 在 retry loop 內能正確產生 attempt raw 檔名與 sidecar metadata。
...
- 新增 `archive_git_state()`,在**每次 `work()` 之後、commit 之前**存:
  ...
  - `NNN-git-diff.patch`:工作者本輪的變更。**不得讓 snapshot 永久改動 index(見 review #8-5)**:優先用不碰 index 的方式保存 tracked diff + untracked 檔內容(例如 `git diff --binary HEAD --` 搭配逐一 `git diff --no-index /dev/null <untracked>` 或等價 helper)。若實作選擇 `git add -N` 來納入未追蹤檔,必須在 snapshot 後清掉 intent-to-add 狀態並測試 `git status --porcelain` 前後一致,避免 archive 動作本身改變後續 commit 行為。
...
- **`auto-workflow.sh`**
  ...
  - `metric()`(:86):CSV 標頭與每列**尾端追加** `model,model_args,generated_at`(保持前 7 欄位置不變),且所有欄位經 `write_csv_row` 轉義後寫入。
  ...
  - `engine_call`(:257):介面改為 `engine_call <role> <engine> <artifact_slug> <engine_fn> <prompt>`;每個 attempt 呼叫後即刻 `archive_snapshot` 當次 `ENGINE_OUT`(檔名帶 `attempt-<n>-rc<rc>`);retry 前後加 `log_section`。
  - `work`(:282):呼叫前落 worker prompt;以明確 role/engine/artifact_slug 呼叫 `engine_call`;呼叫後 `archive_git_state` + output/最終 raw 快照。
  - `run_review`(:403):`compose_review_prompt` → 落 prompt → 以明確 role/engine/artifact_slug 傳入 `engine_call`/`r_*`;審查後快照 `review.md`/`verdict.json`。
  - `begin_stage`/`gate_loop`/`commit_work`/`finish`/**startup 設定摘要**/**`check_protected`**/**`human_gate_spec`** 加 `log_section`。
  - `main()`(:576):**先擷取 task 來源中繼資料(在 `:586` 覆寫前、`setup_workspace` cd 前解析絕對路徑)**;`setup_workspace` 後 `mkdir -p "$WF" "$WF_RUN/logs" "$SPEC_DIR"` → **`init_live_state`** → 寫 `NNN-run-metadata.json`、`$LOG.meta.json`、`archive_task`、`latest-run.txt`。
  - 標頭環境變數文件補上 `RUNS_DIR`。
...
- **`tests/helpers.test.sh`**
  - `metric` 測試(:121):改指向 `$WF_RUN` 下的 metrics 路徑;**斷言新標頭尾端含 `model,model_args,generated_at` 且前 7 欄位置不變**;新增含逗號/引號/空白的 `model_args` 案例,確認 CSV escaping 正確且 `awk -F,` 既有欄位仍可用於前 7 欄。
  ...
  - `archive_git_state`:含未追蹤檔案例;驗證 patch 有保存未追蹤內容,且 snapshot 前後 `git status --porcelain` 一致(沒有殘留 intent-to-add)。
  - `engine_call`:stub 引擎驗證 role/engine/artifact_slug 會出現在 attempt raw 檔名與 metadata,多次 retry 各 attempt 都不覆蓋。

- **`tests/e2e/run.sh`**
  - 以 `.workflow/latest-run.txt` 解析 run 目錄;metrics/log 斷言改指該目錄。
  - 新增斷言:run 目錄存在、`task-source`/`task` 已保存、prompt/output 產物存在且序號遞增、metadata 齊備、`latest-run.txt` 指向本次 run、**每輪 `git-status`/`git-diff` 快照存在**、**metrics 新欄位齊備且 CSV 可解析**、**`logs/001-run.log.meta.json` 存在且含 run/generator 資訊**。
...
- **Commit 2** `feat(workflow): archive per-run intermediate artifacts`
  ...
  - `compose_review_prompt` 重構(exact reviewer prompt);`engine_call` 改明確 archive context 參數;`metric` 追加 model/model_args/generated_at 欄且做 CSV escaping;新增 `$LOG.meta.json`。
  - 更新 helper tests。
...
3. 手動小型 run(可設 `RUNS_DIR=/tmp/wf-archive` 驗證可設定性),完成後檢查:
   ...
   - 每份產物有對應 `.meta.json`,欄位含 engine/model/model_args/role/stage/round/time;`logs/001-run.log.meta.json` 存在;`metrics.csv` 尾端含 `model,model_args,generated_at` 且含特殊字元的 args 不破壞 CSV。
   ...
   - `RUNS_DIR` 指到 `.workflow` 外時,`.workflow/` 仍會被建立,live 檔與 `.workflow/.gitignore` 正常寫入。
...
- metrics 所有資料列一律經 CSV escaping;若未來需要更複雜查詢,可另加 JSONL,但本次仍保留 CSV 作為摘要格式。
- log 除了區段標頭外,必須有 `$LOG.meta.json`,確保 log 檔本身離開 run 目錄後仍保有生成者與時間 attribution。
- `archive_git_state` 不得留下 index 副作用;若使用 `git add -N`,必須在函式內復原並用測試鎖住。
