...
|---|---|---|
| The default `TOOLS` allowlist is detected from the workspace | Python-only addition, not a port gap: bash allowed `git` and the three Go commands, so a Claude slot on an npm, Cargo or Python project was denied the very commands its detected gate runs, and the run stalled on a permission prompt nobody could answer. Detection adds the rule set of every ecosystem the workspace holds, as a union rather than the gate's first match, because one repository can be several; a workspace matching none keeps the whole union, since its gate is user-set and a narrower guess would only remove rules. An explicit `TOOLS` still replaces the value rather than extending it. | `tests/test_gates.py::test_detect_tools_is_a_union_where_the_gate_is_a_first_match`, `tests/test_cli.py::test_startup_narrows_tools_to_the_detected_ecosystem` |
| Python projects get a detected pytest quality gate | Python-only addition, not a port gap: bash detected Go, npm and Cargo only, so a Python project ran with every deterministic gate disabled and nothing but an agent's word that its tests passed. Claimed only when a marker file, a statement that pytest is the runner, and at least one test file all hold, because pytest exits 5 on an empty collection and a gate would read that as a failure to repair. The command follows whatever owns the environment (`uv.lock`, `poetry.lock`, the project's `.venv`) before falling back to the `python` spelling this machine has; `sys.executable` is never used, since it runs the workflow rather than the project under test. | `tests/test_gates.py::test_detect_gate_python_prefers_the_tool_that_owns_the_environment`, `::test_detect_gate_python_without_pytest_or_tests_claims_nothing` |
...
| Extra agent arguments use POSIX shell quoting | Bash used unquoted word splitting, which could not preserve embedded spaces. Python parses every built-in and custom agent argument variable with `shlex.split(..., posix=True)` before passing an argv list directly to `Popen`; Windows follows the same quoting rules. | `tests/test_agents.py::test_generic_worker_passes_args_and_prompt_as_final_arg`, `::test_validate_agents_rejects_unclosed_quotes` |
| Codex and Agy resume by captured IDs instead of global recency | The Bash workflow uses Codex `--last` and Agy `--continue`, so it must reject identical slots. The Python workflow captures Codex `thread_id` and Agy conversation UUIDs, allowing identical built-in commands without cross-session races. | `tests/test_session_resume.py` |
...
| The spec gate can enable Phased ATDD when the reviewer suggests it | Python-only addition, not a port gap: with `PHASES` unset and `IMPORT_PLAN` not set, the spec reviewer writes a fitness judgment to `aac/.run/phased-suggestion.json` and the gate may offer the flip; the snapshot is rewritten atomically so `PHASES` stays consistent across resume. Off whenever `PHASES` is set explicitly or a plan is imported, so the bash-equivalent flow is unchanged. | `tests/test_stageflow.py::test_spec_gate_offer_flips_settings_and_snapshot`, `tests/test_phased_integration.py::test_spec_gate_suggestion_flips_run_to_phased` |
| A separate implementation slot (`IMPL_AGENT`, `IMPL_MODEL`, `IMPL_ARGS`) can run the stage-5 per-task loop | Python-only addition, not a port gap: the retired Bash workflow always used the selected owner for implementation. With all three values empty, Python preserves that behavior. | `tests/test_stageflow.py::test_write_code_routes_only_task_loop_repairs_and_commit_to_impl`, `tests/test_resume_integration.py::test_mid_queue_resume_switches_impl_wrapper_without_rerunning_completed_task` |
| Extra arguments belong to slots | Python takes extra CLI arguments only from `AGENT_A_ARGS`, `AGENT_B_ARGS`, and `IMPL_ARGS`. Command-wide extra-argument variables are rejected even when empty. Settings snapshots use schema 2 and refuse schema 1. The retired Bash workflow shared extra arguments by CLI name. | `tests/test_config.py::test_from_env_rejects_removed_adapter_args_even_when_empty`, `tests/test_runstate_snapshot.py::test_new_snapshots_use_schema_2_and_only_slot_argument_fields`, `::test_schema_1_snapshots_are_rejected_without_migration` |
| Built-in slot arguments reject model flags | Python reserves built-in model flags for `MODEL_A`, `MODEL_B`, or `IMPL_MODEL`, keeping the executed model consistent with archived metadata. The retired Bash workflow did not reject model flags in extra args. Custom-agent args remain pass-through. | `tests/test_agents.py::test_validate_agents_rejects_builtin_model_args`, `::test_validate_agents_rejects_codex_model_config`, `::test_validate_agents_rejects_impl_model_flags_for_builtin_adapters` |
| Claude calls stream `--output-format stream-json` instead of buffering `--output-format json` | Bash showed the user nothing for the whole Claude call, which is indistinguishable from a hung run on a long step. The streaming runner returns the same envelope the buffered call produced, so the session, cost, result, and verdict parsing are unchanged; when no result event arrives it falls back to the whole raw stream, which is what keeps `ratelimit.py` able to detect a quota abort from `agent_out`. `--verbose` is required by the CLI for this format and is therefore reserved in Claude-targeted `AGENT_A_ARGS`, `AGENT_B_ARGS`, and `IMPL_ARGS`. | `tests/test_agents.py::test_claude_stream_writes_raw_and_returns_the_result_envelope`, `::test_claude_stream_without_result_event_falls_back_to_the_raw_stream`, `::test_claude_worker_quota_stream_reaches_agent_out_for_ratelimit` |
| Quota detection reads a narrow per-adapter channel instead of the whole output file | Bash scraped the entire agent output for `rate.?limit`. That file now holds Codex's completed tool calls, including each command's full output, so this repo's own `test_ratelimit_parsing.py` filename made a failed Codex call look quota limited and cost the full backoff. Detection now reads `AgentResult.quota_text`: Claude's envelope, Codex's `error` / `turn.failed` / non-JSON lines, and Agy's whole output, since Agy has no event boundaries to narrow to. | `tests/test_agents.py::test_command_output_that_matches_the_regex_is_not_a_quota_signal`, `::test_real_quota_error_still_reaches_the_quota_channel` |
| Claude's rate limit is detected from `api_error_status` and waited out with its reported `resetsAt` | Bash could only see text: the non-streaming envelope has no rate-limit field and `rate_limit_event` exists only in the stream. With the field available, the status decides on its own so quota wording in Claude's own reply cannot trigger a false wait, and an exact epoch sidesteps the wall-clock formats. The event fires on successful calls too, so it only ever supplies a wait, never the decision. | `tests/test_ratelimit_parsing.py::test_envelope_status_429_is_rate_limited_without_any_wording`, `::test_envelope_with_another_status_is_not_rate_limited`, `tests/test_agent_call.py::test_reported_epoch_sets_the_wait_instead_of_parsed_wording` |
| Codex's captured command output is elided from `agent_out` | Bash dumped whatever the CLI printed. The Python adapter recorded each `item.completed` verbatim, so every command's captured output was written twice: once as the raw line in `raw_out`, again re-serialised into `agent_out`, where it was 73-80% of the file on real streams. `agent_out` is the readable artifact, so the value is replaced with its character count and a pointer to the `.cli.raw` artifact, while `command`, `exit_code`, and `status` survive. | `tests/test_agents.py::test_render_codex_event_elides_captured_command_output`, `::test_render_codex_event_leaves_payloads_without_bulk_untouched` |
| Codex echoes a one-line summary for every tool call | Bash echoed only Codex `agent_message` items, so the stretches where Codex was running commands or editing files were dark. `item.started` is echoed because it fires when the call begins, unlike `item.completed`. The summary replaces the raw JSON that `item.started` previously dumped into `agent_out`; every other event keeps its old rendering, so quota wording still reaches `ratelimit.py` and unknown events stay diagnosable. | `tests/test_agents.py::test_codex_stream_echoes_tool_activity_and_writes_summaries`, `::test_render_codex_event_command_summary_strips_the_shell_wrapper`, `::test_render_codex_event_records_other_events_without_echoing` |
| Streamed agent output is prefixed with its slot and command | Bash echoed agent lines bare, so an agent writing `### heading` was indistinguishable from a workflow human checkpoint, and with three agents it was not clear who was speaking. The prefix is added at print time only, so archived artifacts and the run log stay byte-identical to the agent's own output. | `tests/test_agents.py::test_streaming_prefixes_the_echo_but_not_the_file`, `tests/test_style.py::test_agent_line_is_painted_with_the_agent_color` |
