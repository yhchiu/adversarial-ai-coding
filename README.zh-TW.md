...
| `AAC_LANG` | 未設定(英文) | workflow 自己印在終端機上的人話語言:`en` 或 `zh-TW`(`zh_TW`、`zh-Hant*`、`zh-HK` 也會選繁中)。未設定或無法辨識時維持英文。套件不讀 `LANG`、`LC_*` 或 Windows UI culture。`scripts/aac` 與 `scripts/aac.cmd` 在未設定且系統 locale 是繁中時會寫入 `AAC_LANG=zh-TW`;`uv run adversarial-ai-coding` 除非你自己設定,否則維持英文。run log、prompt 與 artifact 維持英文。 |
