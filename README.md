...
| `AAC_LANG` | unset (English) | Language of the workflow's own terminal messages: `en` or `zh-TW` (`zh_TW`, `zh-Hant*`, and `zh-HK` also select Traditional Chinese). Unset or unknown values stay English. The package does not read `LANG`, `LC_*`, or the Windows UI culture. `scripts/aac` and `scripts/aac.cmd` set `AAC_LANG=zh-TW` when it is unset and the OS locale is Traditional Chinese; `uv run adversarial-ai-coding` stays English unless you set this. Run logs, prompts, and artifacts stay English. |
