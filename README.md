...
| `GATE_CMD` | auto-detected | Full quality gate. Go projects use `go build ./... && go vet ./... && go test ./...`, npm projects with a `test` script use `npm test`, Cargo projects use `cargo test`, and Python projects that name pytest and have test files run it through whatever owns their environment: `uv run pytest` with a `uv.lock`, `poetry run pytest` with a `poetry.lock`, the project's own `.venv` interpreter, else `python -m pytest` (or `python3`, whichever this machine has). Projects without a detected gate skip deterministic gates unless you set it. |
| `BUILD_GATE_CMD` | auto-detected | Lightweight per-task build gate. Go projects use `go build ./...`, Cargo projects use `cargo build`, and projects without a detected build gate - Python among them, having no compile step worth gating - skip this per-task gate unless you set it. |
